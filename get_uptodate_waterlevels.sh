...
curl_init_req () {
    curl 'http://www.isdm-gdsi.gc.ca/isdm-gdsi/twl-mne/inventory-inventaire/data-donnees-eng.asp?user=isdm-gdsi&region=LAU&tst=1&no='${STATION_ID} \
    ...
    -H 'Referer: http://www.isdm-gdsi.gc.ca/isdm-gdsi/twl-mne/inventory-inventaire/interval-intervalle-eng.asp?user=isdm-gdsi&region=LAU&tst=1&no='${STATION_ID} \
    -H 'Upgrade-Insecure-Requests: 1' \
    --data 'start_period=1960%2F01%2F01&end_period='$(date +%Y%%2F%m%%2F%d)'&resolution=d&time_zone=l&pcode=slev&datum=c' | \
    tee /tmp/uptodate_waterlevels_init_req
}
...
if [[ "$1" == "check-parse_csv_addr" ]]; then
    echo "Checking parse_csv_addr"
    parse_csv_addr
else
    _main
fi
